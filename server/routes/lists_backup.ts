...
router.get('/:listId', authenticate, async (req, res) => {
  try {
    ...
    const [list] = await db
      ...
      .where(restaurantLists.id)listId);
    ...
    const items = await db
      ...
      .leftJoin(restaurants, restaurants.id)restaurantListItems.restaurantId)
      .where(restaurantListItems.listId)listId)
...
router.put('/:listId', authenticate, async (req, res) => {
  try {
    ...
    const [existingList] = await db
      ...
      .from(restaurantLists)
      .where(
        restaurantLists.id)listId)
        .and(restaurantLists.createdById)userId)
      );
...
router.put('/:listId/items/:itemId', authenticate, async (req, res) => {
  try {
    ...
    const [existingItem] = await db
      ...
      .from(restaurantListItems)
      .where(
        restaurantListItems.id)itemId)
        .and(restaurantListItems.listId)listId)
      );
    ...
    const [updatedItem] = await db
      ...
      .where(restaurantListItems.id)itemId)
      .returning();
